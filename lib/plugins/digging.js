...
function inject(bot) {
  ...
  async function dig(block, forceLook, digFace) {
    ...
    console.log({mineflayer: {digging: {event: 'dig called', block: block?.position}}});
    ...
    // Hack to get more consistent digging of fast breaking blocks
    const waitTime = 50 + bot.digTime(block)
    bot.emit('diggingTimeEstimate', waitTime)
    ...
    bot.stopDigging = () => {
      ...
      // logger.info({mineflayer: {digging: 'stopDigging called'}})
      console.log({mineflayer: {digging: 'stopDigging called', removeEvent: eventName}})
    ...
    function onBlockUpdate(oldBlock, newBlock) {
      ...
      console.log({mineflayer: {digging: 'onBlockUpdate called', removeEvent: eventName}})
      bot.removeListener(eventName, onBlockUpdate)
      clearInterval(swingInterval)
      clearTimeout(waitTimeout)
      swingInterval = null
