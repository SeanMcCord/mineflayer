...
function inject(bot, {version, hideErrors}) {
  ...
  // Vanilla minecraft client resets the action id to 0 when a window is opened.
  // The inventory does not send these packets.
  function resetWindowsActionNumber() {
    nextActionNumber = 0;
  }
  ...
  function closeWindow(window) {
    // We must increment the actionId when a non inventory window is closed.
    // This is the behavior of the vanilla client.
    if (window.id !== 0) {
      createActionNumber();
      // This second one is a hack to account for the fact that open_window does not get sent by the
      // vanilla client when opening the inventory.
      createActionNumber();
    }
