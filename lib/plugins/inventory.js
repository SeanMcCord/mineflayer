...
function inject(bot, { version, hideErrors }) {
  ...
  async function putSelectedItemRange(start, end, window, slot, noWaiting = false) {
    ...
    // TODO handle the case of the server putting the item in a different slot than we antisipated.

    console.log('putSelectedItemRange Function Start')
    while (window.selectedItem) {
      ...
      if (item && item.stackSize !== item.count) { // something to join with
        if (noWaiting) {
          ...
          console.log({ event: 'await window click and update', slot: item.slot })
          // console.log({ window })
          console.log({ slotBefore: window.slots[item.slot] })
          // window.on(`updateSlot:${item.slot}`, (...args) => {
          //   console.log({ slot: item.slot, UPDATE: args })
          // })

          const ac = new AbortController();
          const updatePromise = once(window, `updateSlot:${item.slot}`, { signal: ac.signal })
          const updatePromiseWithTimeout = new Promise((resolve, reject) => {
            updatePromise.then(() => resolve()).then(() => console.log(`updatePromise Done for slot ${item.slot}`)).catch(console.log)

            setTimeout(() => {
              ac.abort()
              console.log(`updatePromise Timedout slot ${item.slot}`)
              resolve()
            }, 500);
            // bump timeout from 500 to 10000 to test furnaces
          });
          // TODO: find out why the minecraft server is not sending back confirmations on moving items onto existing stacks
          const clickWindowPromise = clickWindow(item.slot, 0, 0)
          clickWindowPromise.then(() => console.log('clickwindowPromise done')).catch(console.error)
          // console.log({ window })
          console.log({ slotAfter: window.slots[item.slot] })
          await Promise.all([clickWindowPromise, updatePromiseWithTimeout])
        }
        ...
        if (emptySlot === null) { // no room left
          ...
          if (noWaiting) {
            ...
            // window.on(`updateSlot:${emptySlot}`, (...args) => {
            //   console.log({ UPDATE: args })
            // })
            console.log({ waitOnSlot: emptySlot })
  ...
  async function clickWindow(slot, mouseButton, mode) {
    ...
    if (!window.transactionRequiresConfirmation(click)) {
      console.log({ confirmTransactionSlot: slot })
  ...
  async function putAway(slot, noWaiting = false) {
    ...
    console.log({ putSelected: { start, end } })
    // console.log({ window })
    console.log('put SelectedItem start')
    // try not waiting
    await putSelectedItemRange(start, end, window, null, noWaiting)
    console.log('put SelectedItem end')
    console.log('put away promise start')
    await promisePutAway
    console.log('put away promise end')
  }
  ...
  function prepareWindow(window) {
    if (!windowItems || window.id !== windowItems.windowId) {
      ...
      for (let i = 0; i < windowItems.items.length; ++i) {
        ...
        console.log({ prepareWindowUpdateSlot: i })
  ...
  bot._client.on('set_slot', (packet) => {
    ...
    console.log({ set_slotUpdateSlot: packet.slot })
  ...
  bot._client.on('window_items', (packet) => {
    ...
    for (let i = 0; i < packet.items.length; ++i) {
      ...
      console.log({ windows_itemsUpdateSlot: i })
      window.updateSlot(i, item)
    }
    updateHeldItem()
  ...
  bot.asyncClickWindow = clickWindow
