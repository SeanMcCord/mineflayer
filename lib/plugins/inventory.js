...
function inject(bot, {version, hideErrors}) {
  ...
  async function putSelectedItemRange(start, end, window, slot, waitForSlotUpdate = false) {
    ...
    while (window.selectedItem) {
      ...
      if (item && item.stackSize !== item.count) { // something to join with
        if (waitForSlotUpdate) {
          console.log({waitForStackSlot: item.slot, inventory: bot.inventory})
        ...
        if (emptySlot === null) { // no room left
          ...
          if (waitForSlotUpdate) {
            console.log({placeInEmptySlotAndWait: emptySlot, selectedItem: window.selectedItem})
            await Promise.all([clickWindow(emptySlot, 0, 0), once(window, `updateSlot:${emptySlot}`)])
          } else {
            console.log({placeInEmptySlot: emptySlot, selectedItem: window.selectedItem})
  ...
  async function putAway(slot, waitForSlotUpdate = false) {
    const window = bot.currentWindow || bot.inventory
    const promisePutAway = once(window, `updateSlot:${slot}`)
    await clickWindow(slot, 0, 0)
    const start = window.inventoryStart
    const end = window.inventoryEnd
    await putSelectedItemRange(start, end, window, null, waitForSlotUpdate)
  ...
  bot._client.on('set_slot', (packet) => {
    ...
    console.log({slot: window.id, oldItemName: oldItem == null ? null : oldItem.name, newItemName: newItem == null ? null : newItem.name})
